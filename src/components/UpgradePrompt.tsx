...
export default function UpgradePrompt({ variant = "modal", onUpgrade, className = "" }: UpgradePromptProps) {
  ...
  const features = ["Unlimited story generation", "No daily limits", "Priority support", "Early access to new features"]
    ...
    return (
      <Alert className={`border-orange-200 bg-orange-50 ${className}`}>
        ...
        <AlertDescription className="flex items-center justify-between">
          <div>
            <span className="font-medium text-orange-800">You&apos;ve reached your story limit!</span>
            <span className="text-orange-700 ml-2">Upgrade to Pro for unlimited stories.</span>
          </div>
          <Button
          ...
          >
            {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Upgrade Now"}
          </Button>
        </AlertDescription>
      </Alert>
    )
  ...
  return (
    <Card className={`shadow-lg border-2 border-orange-200 ${className}`}>
      <CardHeader className="text-center bg-orange-50">
        ...
        <CardTitle className="text-2xl font-bold text-gray-900">Unlock Unlimited Stories</CardTitle>
